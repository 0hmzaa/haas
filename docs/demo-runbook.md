...
- `X402_PAYMENT_FEE_PAYER`: optional Hedera facilitator fee payer account (`0.0.x`) injected into `paymentRequirements.extra.feePayer` when `X402_PAYMENT_NETWORK` is Hedera.
...
- `REAL_PAYER_PRIVATE_KEY`: payer private key used by the local helper to generate a fresh Hedera x402 `paymentHeader`.
- `REAL_ALLOW_WEBHOOK_FALLBACK`: set `true` only when you intentionally want legacy signed-webhook funding fallback.
- `REAL_ORDER_AMOUNT`, `REAL_ORDER_CURRENCY`, `REAL_CLIENT_ID`: scenario parameters (`REAL_ORDER_AMOUNT` defaults to `1.00` HBAR).
...
- `REAL_PAYMENT_REQUIREMENTS_FILE`: optional output path where `demo:real-e2e` stores `paymentRequirements` JSON for the current run.
- If `REAL_X402_PAYMENT_HEADER` is omitted and `REAL_PAYER_PRIVATE_KEY` is present, the script auto-generates a Hedera-compatible header from `paymentRequirements`.
- If `REAL_X402_PAYMENT_HEADER` is omitted and no private key is provided, fallback is blocked by default; set `REAL_ALLOW_WEBHOOK_FALLBACK=true` to allow signed webhook fallback (requires `REAL_HEDERA_TX_ID`).

Mirror verification behavior for real direct-submit:
- When `X402_VERIFY_HEDERA_TX=true`, the backend retries Mirror Node lookup for indexing lag.
- If lookup still fails for transient reasons, funding is accepted with `hederaTxVerified=false` and can be reconciled later.
